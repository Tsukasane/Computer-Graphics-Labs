...
void OpenGLWidget::initializeGL() {
    ...
    float vertices[] = {
            //顶点坐标       纹理坐标       颜色坐标
           -1.0f, -0.5f,   0.0,0.0,   1.0f,0.0f,0.0f,  //0.0, 0.0
            1.0f, -0.5f,   4.0,0.0,   0.0f,1.0f,0.0f,  //1.0, 0.0
            1.0f,  0.5f,   4.0,2.0,   0.0f,0.0f,1.0f, //1.0, 1.0

           -1.0f, -0.5f,   0.0, 0.0,  1.0f,1.0f,0.0f, //0.0, 0.0
            1.0f,  0.5f,   4.0, 2.0,  1.0f,0.0f,1.0f, //1.0,1.0
           -1.0f,  0.5f,   0.0, 2.0,  0.0f,1.0f,1.0f  //0.0,1.0
       };
    ...
    // 指定 位置坐标 属性的存储位置与格式
    glVertexAttribPointer(0, 2, GL_FLOAT, GL_FALSE, 7 * sizeof(GL_FLOAT), (const void*)0);
    glEnableVertexAttribArray(0);

    // 指定 纹理坐标 的存储位置与格式
    glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, 7 * sizeof(GL_FLOAT), (const void*)(2 * sizeof(GL_FLOAT)));
    glEnableVertexAttribArray(1);

    // 指定 颜色坐标 的存储位置与格式
    glVertexAttribPointer(2, 3, GL_FLOAT, GL_FALSE, 7 * sizeof(GL_FLOAT), (const void*)(4 * sizeof(GL_FLOAT)));
...
void OpenGLWidget::paintGL() {
    ...
    glViewport(0, 0, this->width()*2, this->height()*2);  //视窗x2
